...
export default function AwardsPage() {
  ...
  const filtered = items.filter((a) => {
    ...
    const mq =
      ...
      (a.company && a.company.includes(search)) ||
      (a.summary && a.summary.includes(search));
    return mp && mq;
  });
  ...
  return (
    <PageLayout
      title="中标公示"
      description="光伏储能项目中标结果公示，每条均标注原始出处"
    >
      <div className="grid grid-cols-2 gap-4 mb-6">
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-xs text-slate-500">中标公示</p>
          </CardContent>
        </Card>
        <Card className="border-0 shadow-sm">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-green-600">
              {filtered.filter((a) => a.amount).length}
            </p>
            <p className="text-xs text-slate-500">含金额信息</p>
          </CardContent>
        </Card>
      </div>
