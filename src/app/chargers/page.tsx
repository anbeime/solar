...
export default function ChargersPage() {
  ...
  return (
    <PageLayout title="充电设施" description="充电站与充电桩相关资讯，每条均标注原始出处">
      <div className="flex flex-wrap gap-2 mb-4">
        <div className="relative flex-1 min-w-[180px]">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
          <Input placeholder="搜索标题、公司..." value={search} onChange={e => setSearch(e.target.value)} className="pl-8 h-9 text-sm" />
        </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {filtered.map(c => (
            <Card key={c.id} className="hover:shadow-md transition-shadow border border-slate-200/80 shadow-sm">
