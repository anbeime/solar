...
export default function BiddingPage() {
  ...
  return (
    <PageLayout title="招标公告" description="最新光伏储能招标公告，每条均标注原始出处">
      <SimpleStatCards items={[
        { value: filtered.length, label: '招标公告', color: 'text-purple-600' },
        { value: activeCount, label: '报名中', color: 'text-amber-600' },
        { value: filtered.filter(b => b.amount).length, label: '含预算信息', color: 'text-blue-600' },
      ]} />

      <div className="flex flex-wrap gap-2 mb-4">
        <div className="relative flex-1 min-w-[180px]">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
          <Input placeholder="搜索招标标题、公司..." value={search} onChange={e => setSearch(e.target.value)} className="pl-8 h-9 text-sm" />
        </div>
        <Select value={province} onValueChange={setProvince}><SelectTrigger className="w-24 h-9 text-sm"><SelectValue /></SelectTrigger><SelectContent>{provinces.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent></Select>
        <Select value={status} onValueChange={setStatus}><SelectTrigger className="w-24 h-9 text-sm"><SelectValue /></SelectTrigger><SelectContent>{['全部', '报名中', '已截止'].map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select>
      </div>

      <div className="space-y-3">
        {filtered.map(b => (
          <Card key={b.id} className="hover:shadow-md transition-shadow border border-slate-200/80 shadow-sm">
