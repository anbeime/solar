...
export default function ProvincePage() {
  ...
  return (
    <PageLayout title="省份分析" description="按省份统计光伏储能项目分布">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        {[
          ...
          <button key={s.key} onClick={() => setSortBy(s.key)} className={`px-3 py-1 rounded-full text-xs transition-colors ${sortBy === s.key ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
            {s.label}
          </button>
        ))}
      </div>
